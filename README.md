...
`chart` produces identical output to the last 16 lines of ESR's
[ascii](https://gitlab.com/esr/ascii) program when run with no
arguments:
...
The difference is that `chart` produces colorized output, while
`ascii` produces non-colorized output.  I personally find the
colorized output a bit easier to read.  If you don't want
colorized output, just use `ascii`.

## Building

`chart` can be built in the normal way with either Cabal or Stack.
